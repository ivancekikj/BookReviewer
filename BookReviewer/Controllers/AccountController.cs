...
namespace BookReviewer.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult PersonalReviews()
        {
            ...
            ICollection<Review> books = db.Users
                .Find(userId)
                .Reviews
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            ApplicationUser user = db.Users
                .SingleOrDefault(u => u.UserName.Equals(model.Username));

            if (user != null
                && UserManager.IsInRole(user.Id, "Administrator")
                && user.AdminApproval != null
                && !user.AdminApproval.IsApproved)
            {
                ModelState.AddModelError("Username", "Admin approval hasn't been granted!");
                return View(model);
            }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (model.ChosenRole.Equals("Administrator"))
                    {
                        AdminApproval approval = new AdminApproval { Id = user.Id, IsApproved = false };
                        db.AdminApprovals.Add(approval);
                        db.SaveChanges();
                    }
                    else
                    {
